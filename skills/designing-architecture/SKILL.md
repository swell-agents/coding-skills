...
description: Design technical architecture for a new feature, library selection, or data layer before implementation. Use when starting a new feature, choosing a library, picking a design pattern, or designing a database schema. Researches the ecosystem (awesome-lists, GitHub, package registries, official docs, ThoughtWorks Radar, DB-Engines, CNCF Landscape), evaluates candidates with consistent dimensions (stars, recency, license, dep footprint, docs quality), compares alternatives, and emits a TDD-ready implementation plan with components, data flow, and an ASCII architecture diagram. Covers application architecture and database architecture (relational, document, key-value, time-series, graph, search). Designs only — does not write the implementation; hands the plan to running-tdd-cycles. Composes with python-conventions, go-conventions, solidity-conventions, engineering-philosophy.
