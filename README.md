...
- **Workflow skills** — what to do (verbs): `running-tdd-cycles`, `reviewing-changes`, `designing-architecture`, `creating-block-issues` (Spec Kit projects — one issue per `tasks.md` PR-stack block), `committing-changes` (also installs an optional PR-size CI gate that fails PRs over 1000 changed lines, excluding tests/docs/lockfiles/generated), `implementing-blocks` (Spec Kit projects — one PR-stack block end-to-end).
