...
- `.claude-plugin/plugin.json`: version bumped `2.1.2` -> `2.1.3`.

## [2.1.2] - 2026-05-27

### Changed

- **`engineering-philosophy` -> DRY, repo-wide reuse.** DRY now states reuse spans the repo: prefer an existing sibling idiom over a second variant.
- **`reviewing-changes` -> Pass 1 (Code quality), new bullet.** Added "Redundant entity & local-pattern reuse": flag a new function, accessor, or code path the repo already provides (an auto-generated getter, a wider getter that subsumes a narrower one, an idiom a sibling file implements) and prefer reusing the existing form. Closes the gap where the rubric only caught duplication within the diff, not divergence from an idiom already in the tree. Picked up automatically by `code-reviewer`, which reads the skill verbatim. No new skill, agent, or command; wording-only.
